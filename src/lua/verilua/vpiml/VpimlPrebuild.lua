...
local function alloc_handle(signal_info)
    ...
    assert(not handle_to_signal_info_map[handle], f("[VL_PREBUILD] [alloc_handle] Handle `%d` already exists", handle))
...
vpiml.vpiml_handle_by_name = function (name)
    ...
    assert(signal_info, f("[VL_PREBUILD] [vpiml_handle_by_name] No handle found for `%s`", name))
...
vpiml.vpiml_get_hdl_type = function(handle)
    ...
    assert(signal_info, f("[VL_PREBUILD] [vpiml_get_hdl_type] No signal info found for handle `%d`", handle))
    return signal_info[3]
end

vpiml.vpiml_get_signal_width = function(handle)
    local signal_info = handle_to_signal_info_map[handle]
    assert(signal_info, f("[VL_PREBUILD] [vpiml_get_signal_width] No signal info found for handle `%d`", handle))
