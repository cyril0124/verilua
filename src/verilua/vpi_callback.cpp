...
VERILUA_PRIVATE static PLI_INT32 next_sim_time_callback(p_cb_data cb_data) {
    ...
    // VL_INFO("before clean => {}\n", env.vec_value_cache.size());
    env.vec_value_cache.clear();
#endif

    // VL_INFO("[cbNextSimTime] leave next_sim_time_callback()\n");
    return 0;
}

VERILUA_PRIVATE static PLI_INT32 readwrite_synch_callback(p_cb_data cb_data) {
    VL_INFO("[cbReadWriteSynch] enter readwrite_synch_callback()\n");

    VL_FATAL(false, "TODO:");

    VL_INFO("[cbReadWriteSynch] leave readwrite_synch_callback()\n");
    return 0;
}
...
VERILUA_PRIVATE void register_readwrite_synch_calllback(void) {
    ...
    VL_INFO("register_readwrite_synch_calllback()\n");
    registered = true;
}
...
VERILUA_PRIVATE void register_next_sim_time_calllback(void) {
    ...
    VL_INFO("register_next_sim_time_calllback()\n");
    registered = true;
}