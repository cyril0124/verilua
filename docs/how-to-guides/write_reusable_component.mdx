...
如果你还不熟悉 `local Monitor = class()`、`_init`、`Monitor(...)` 这些 `pl.class` 的基本写法，建议先阅读 [使用 pl.class 进行面向对象编程](./oop_with_pl_class)。
...
```mermaid
flowchart LR
    subgraph Without AliasBundle
        M1[Monitor uses valid and value]
        A1[DUT_A signals: valid and value]
        B1[DUT_B signals: vld and value_2]
        M1 --> A1
        M1 -. rewrite needed .-> B1
    end

    subgraph With AliasBundle
        M2[Monitor uses aliases valid and value]
        AB[AliasBundle surface]
        A2[DUT_A mapping]
        B2[DUT_B mapping]
        M2 --> AB
        A2 --> AB
...
:::tip[进一步阅读 `TypeExpect`]
如果你想系统了解 `expect_chdl()`、`expect_abdl()`、`fake_chdl` 与位宽约束的完整语义，可以查看 [运行时类型校验（TypeExpect）参考文档](../reference/type_expect)。
:::
...
如果你需要单独查阅 `TypeExpect` 的接口与行为说明，可以继续阅读 [运行时类型校验（TypeExpect）参考文档](../reference/type_expect)。
