...
- `env.posedge(...)` / `env.negedge(...)`
    ...
    对全局的 `clock` 这个 `chdl` 的 `posedge` 函数进行封装，其使用方式和 [`<chdl>:posedge(...)`](../reference/data_structure/callable_hdl.mdx#chdl-posedge) / [`<chdl>:negedge(...)`](../reference/data_structure/callable_hdl.mdx#chdl-negedge) 是一样的。
...
- `env.expect_happen_until(limit_cycles, func)`
    ...
    检查 `func` 在 `limit_cycles` 轮检查内是否发生，如果发生则立即返回，否则会触发 `assert` 错误，这在具体编写验证代码的时候比较常用，用来检查特定信号是否在预期时间内发生。

    这里依赖 [`<chdl>:posedge_until(...)`](../reference/data_structure/callable_hdl.mdx#chdl-posedge-until) 的语义：进入等待前会先立即检查一次 `func`，只有返回 `false` 时才会继续等待下一个 `posedge`。因此如果条件在调用时已经成立，那么这个函数会直接通过，而不需要先等一个时钟周期。

- `env.expect_not_happen_until(limit_cycles, func)`

    和 `env.expect_happen_until(limit_cycles, func)` 作用相反。

    同样需要注意第一次检查是立即发生的；如果条件在调用时已经成立，那么这个函数会立刻触发断言失败。
