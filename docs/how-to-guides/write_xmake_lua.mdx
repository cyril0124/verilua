...
target("my_test", function()
    ...
    add_files("src/design.v", "src/main.lua")   -- RTL 和 Lua 文件
...
target("wave_analysis", function()
    ...
    add_files("path/to/waveform.vcd", "analyze.lua") -- 波形文件和 Lua 脚本
...
- `add_files(...)`：HVL 中通常添加 RTL 文件，以及需要参与工程管理的 Lua/C/C++ 文件；WAL 中添加波形文件和需要复用的 Lua 模块。`cfg.lua_main` 指向的 Lua 入口脚本通常也会放在这里，但不是强制要求。WAL 模式支持 `.vcd`、`.fst`、`.fsdb` 三种波形格式，且**只能添加一个波形文件**，添加多个会报错。
...
target("counter_test", function()
    ...
    add_files("src/Counter.v", "src/main.lua")
...
target("wave_analysis", function()
    ...
    add_files("path/to/waveform.vcd", "analyze.lua")
