...
- `add_files(...)`：HVL 中通常添加 RTL 文件，以及需要参与工程管理的 Lua/C/C++ 文件；WAL 中添加波形文件和需要复用的 Lua 模块。`verilua.lua_main` 指向的 Lua 入口脚本不需要额外放进 `add_files(...)`。WAL 模式支持 `.vcd`、`.fst`、`.fsdb` 三种波形格式，且**只能添加一个波形文件**，添加多个会报错。
...
| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `set_values("verilua.user_cfg", "<file>", ...)` | string 或 string 列表 | - | 指定一个或多个用户自定义 Lua 配置文件；返回表的字段按声明顺序合并到运行期全局 `cfg` 中，后者覆盖前者，`srcs` 和 `deps` 会追加 |
...
- 更完整的 `verilua.*` / `<sim>.*` 参数说明：参考 [xmake 参数详细说明](../reference/xmake_params.mdx)
...
**Q1：编译时报错 `unknown toolchain` 或 `unknown rule: verilua`**
A：请先 `source verilua.sh`（或 `activate_verilua.sh`）激活环境，确认 `XMAKE_RCFILES` 指向 `$VERILUA_HOME/scripts/xmakerc.lua`。`add_toolchains` 须使用正确名称（`@verilator`, `@iverilog`, `@vcs`, `@xcelium`, `@wave_vpi`），且对应工具已在 `PATH` 中。
