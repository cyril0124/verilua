...
| `Lua Clock` | Lua 协程 | 支持 | 支持 | 最低 | 需要 `cfg.no_internal_clock`；Verilator 下基于 `await_time*()` 时通常需要 `--timing` |
| `NativeClock` | Rust 原生层 | 有限支持（通过 `stop()` / `restart()`） | 支持 | 高于 Lua，低于 Internal | 需要 `cfg.no_internal_clock`；Verilator 下需要 `--timing`；仅支持 HVL |
...
在使用 `testbench_gen` 的 HVL 场景下，如果没有显式设置 `cfg.no_internal_clock`，Verilua 默认会在生成的 testbench 中自动添加一个时钟驱动逻辑。
...
你可以在 `xmake.lua` 中通过 `cfg.tb_gen_flags` 调整内部时钟的主时钟名和周期：
...
- 需要先禁用内部时钟：`set_values("cfg.no_internal_clock", "1")`
...
1. 使用 `Lua Clock` 或 `NativeClock` 时忘记设置 `cfg.no_internal_clock`
