...
* `dut.inc:set(1)` 的 `set` 方法用于设置信号的值，区别于立即赋值的 `dut.inc = 1`，`set` 方法进行赋值会在下一个时钟边沿到来后才会赋值（更接近 RTL 代码的行为），而立即赋值则会立即赋（具体可以看 [这里](../reference/data_structure/callable_hdl.mdx#set-and-set-imm)）。`dut` 的 `set` 方法同样只能赋值最多 32 bit 位宽的信号。
