...
:::info[HVL - Hardware Verification Language]
**HVL** 是 **Hardware Verification Language**（硬件验证语言）的缩写。

HVL 是专门用于硬件验证场景的语言,能够控制验证平台的运行或描述复杂的验证逻辑。SystemVerilog 就是一种常见的 HVL,而 Verilua 则选择使用 Lua(具体是 LuaJIT)作为 HVL,在 LuaJIT 基础上构建了硬件验证 DSL,使得用户能够使用 Lua 语言编写硬件验证业务代码。
:::
...
<details>
<summary>上述代码的相关语法解释</summary>

* `fork` 这个语句用于创建一个或多个新的 task，Verilua 中所有的 task 能够以类似并行的方式进行调度；
:::tip[创建 task]
推荐直接使用 `fork` 来创建 task，例如:
```lua
fork {
    function ()
        print("fork task 1")
...
<div id="about-dut"></div>

:::tip[关于 dut]
`dut` 是 Verilua 中的一个全局的 table，主要用来提供**临时**的信号访问功能，具体说明见 [全局信号代理（ProxyTableHandle / dut）](../reference/data_structure/proxy_table_handle.mdx)，如果想用 `dut` 来访问 DUT 的**内部信号**(顶层的接口信号不需要)，那么需要以 `dut.u_<top_module_name>.<internal_signal_name>` 的方式访问，例如 `dut.u_Design.value`，这里加上 `u_<top_module_name>` 的原因是因为 Verilua 在编译的时候会根据所提供的 RTL 信息自动创建 Testbench，这个 Testbench 中会例化当前的 DUT，并将其取名为 `u_Design`。下面是自动生成的 Testbench 的部分代码：

```SystemVerilog title="tb_top.sv"
module tb_top;
    // ...
...
:::note[关于 negedge]
类似 `negedge` 的仿真时间控制函数还有 `posedge`、`edge`等，详见[多任务系统 — 注册任务回调](../reference/multi_task.mdx#注册任务回调)，需要注意的是这些函数只能对**位宽为 1 的信号**进行使用！
:::
...
* `dut.reset = 1` 和 `dut.reset = 0` 可以通过 `dut` 给 reset 信号赋值，其行为与 `<chdl>.value = <value>` 一致，会在 VPI 的 `cbReadWriteSynch` 阶段统一写入，并且支持 `CallableHDL.value` 的宽度感知输入类型。
如果需要立即赋值，可以使用 `dut.reset:set_imm(1)` 和 `dut.reset:set_imm(0)`；这里的 imm 是 immediate 的缩写，即立即（关于立即赋值与普通赋值的区别将会在后续的教程中进行介绍，具体可以看 [这里](../reference/data_structure/callable_hdl.mdx#set-and-set-imm)）。
...
* `dut.value:dump_str()` 会将原本`dut.value:dump()` 的输出的内容作为一个返回值进行返回，因此 `dut.value:dump()` 也等价于 `print(dut.value:dump_str())`。
...
* `dut.inc:set(1)` 的 `set` 方法用于设置信号的值。`set` 会先暂存写入值，并在 VPI 的 `cbReadWriteSynch` 阶段统一写入；如果需要直接写入 HDL，请使用 `dut.inc:set_imm(1)` 或 `dut.inc:chdl().value_imm = 1`（具体可以看 [这里](../reference/data_structure/callable_hdl.mdx#set-and-set-imm)）。`dut` 的 `set*` 方法保留历史行为，只支持 Lua number 并按 32-bit 整数写入；如果需要写入 `uint64_t`、多 beat table 或 BitVec 等宽度感知值，请显式使用 `dut.inc:chdl():set(...)`。

* `dut.inc:get()` 的 `get` 方法用于获取信号的值，返回的值是一个 Lua 的 number 类型的值。如果需要 `CallableHDL` 的宽度感知返回值，可以显式使用 `dut.inc:chdl():get()`。

* `sim.finish()` 用于控制仿真器结束仿真。

* `initial { ... }` 用于添加仿真开始执行时调用的函数，而 `final { ... }` 则用于添加仿真结束执行时调用的函数。这两个函数都能添加多个 function 块，例如：
    ```lua
    initial {
        function ()
...
<div id="create-xmake-lua"></div>
## 2. 创建一个 xmake.lua 文件
Verilua 的工程（HVL 和 WAL 场景）使用 `xmake` 来管理，因此需要先在你的工程文件夹中创建一个 xmake.lua 文件。xmake 是一个基于 Lua 的构建工具（类似 makefile，cmake 等），提供了灵活的构建方式。关于 xmake 的使用，可以参考 [xmake 官方文档](https://xmake.io/#/getting_started)。关于 Verilua 中支持的相关 xmake 配置参数，可以查看[此处](../reference/xmake_params.mdx)的介绍。
...
- [安装指南](./install.mdx)：先完成环境安装，再按本文示例创建工程。
- [LuaJIT 与标准 Lua 的常见差异](./luajit_vs_standard_lua.mdx)：了解本文示例中涉及的 LuaJIT 特性，如 `cdata` 和 64 位整数。
- [编写 xmake.lua](../how-to-guides/write_xmake_lua.mdx)：进一步了解 `xmake.lua` 的组织方式和常见配置。
- [xmake 参数说明](../reference/xmake_params.mdx)：查看 `verilua.top`、`verilua.lua_main` 和仿真器参数的完整说明。
- [信号句柄（CallableHDL）](../reference/data_structure/callable_hdl.mdx)：了解 `dut.clock`、`set`、`set_imm` 等信号操作接口。
- [时钟驱动策略](../how-to-guides/clock_driving.mdx)：了解内部时钟、Lua Clock 和 NativeClock 的差异。
