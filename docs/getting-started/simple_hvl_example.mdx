...
:::tip[关于 dut]
`dut` 是 Verilua 中的一个全局的 table，主要用来提供**临时**的信号访问功能，具体说明见 [ProxyTableHandle](../reference/data_structure/proxy_table_handle.mdx)，如果想用 `dut` 来访问 DUT 的**内部信号**(顶层的接口信号不需要)，那么需要以 `dut.u_<top_module_name>.<internal_signal_name>` 的方式访问，例如 `dut.u_Design.value`，这里加上 `u_<top_module_name>` 的原因是因为 Verilua 在编译的时候会根据所提供的 RTL 信息自动创建 Testbench，这个 Testbench 中会例化当前的 DUT，并将其取名为 `u_Design`。下面是自动生成的 Testbench 的部分代码：
...
- [CallableHDL](../reference/data_structure/callable_hdl.mdx)：了解 `dut.clock`、`set`、`set_imm` 等信号操作接口。
