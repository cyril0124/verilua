...
也可以使用 `dut.reset:set_imm(1)` 和 `dut.reset:set_imm(0)` 来代替这两个方式，这里的 imm 是 immediate 的缩写，即立即（关于立即赋值与普通赋值的区别将会在后续的教程中进行介绍，具体可以看 [这里](../reference/data_structure.mdx#set_and_set_imm)）。
...
* `dut.inc:set(1)` 的 `set` 方法用于设置信号的值，区别于立即赋值的 `dut.inc = 1`，`set` 方法进行赋值会在下一个时钟边沿到来后才会赋值（更接近 RTL 代码的行为），而立即赋值则会立即赋（具体可以看 [这里](../reference/data_structure.mdx#set_and_set_imm)）。`dut` 的 `set` 方法同样只能赋值最多 32 bit 位宽的信号。
