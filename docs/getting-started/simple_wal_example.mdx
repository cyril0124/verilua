...
:::info[WAL - Waveform Analysis Language]
...
```mermaid
flowchart LR
    A[RTL Counter.v] --> B[HVL target gen_wave]
    C[main.lua] --> B
    D[Monitor.lua] --> B
    B --> E[Generate waveform test.vcd]
    E --> F[WAL target sim_wave]
    G[main_for_wal.lua] --> F
    D --> F
    F --> H[Replay waveform and analyze signals]
```
...
```mermaid
flowchart TD
    A[gen_wave HVL run] --> B[testbench_gen creates tb_top]
    B --> C[tb_top wraps DUT and dumps waveform]
    C --> D[test.vcd top module is tb_top]
    D --> E[sim_wave WAL run]
    E --> F[cfg.top = tb_top]
    F --> G[Access testbench signals and DUT internals]
```
...
- [HVL 示例](./simple_hvl_example.mdx)：先生成波形，再对照本文理解 WAL 工作流。
- [仿真器控制](../reference/simulator_control.mdx)：查看 `sim.dump_wave()` 等波形控制接口。
- [xmake 参数说明](../reference/xmake_params.mdx)：查看 WAL 工程中常见的构建参数和运行配置方式。
