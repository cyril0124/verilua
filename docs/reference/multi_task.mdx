...
sequenceDiagram
    ...
    box C/C++ (Hardware Simulator)
        participant VPI as VPI 回调
        participant Sim as 仿真器
    end

    Task ->> Scheduler: 注册回调（如 posedge）
    Scheduler ->> VPI: 注册 VPI 回调
    VPI ->> Sim: 等待事件
    Sim -->> VPI: 事件触发
    VPI -->> Scheduler: 回调触发
    Scheduler ->> Task: 唤醒任务
...
:::warning[`jfork` 必须在 scheduler task 内调用]
...
使用 `join_any` 可以等待多个 `jfork` 任务中**任意一个**完成即返回，返回值为最先完成的那个 `EventHandle`：
```lua
local ehdl1 = jfork {
    fast_task = function ()
        dut.clock:posedge(2)
...
:::note[`join_any` 只接受由 `jfork` 创建的 `EventHandle`，不支持普通的 `EventHandle`]
:::
...
:::warning[`task_group` 必须在 scheduler task 内调用]
...
:::warning[在 `task_group` 内优先使用 `tg:fork()`]
...
`join_all` 采用**动态 drain** 语义：如果被等待的子任务在执行过程中又通过 `tg:fork` 创建了新任务，这些新任务也会被纳入等待范围。循环持续到组内不再有未完成的任务为止。

:::warning[只有 task_group owner task 可以调用 `tg:join_all()`]
子任务可以继续用同一个 `tg` 调用 `tg:fork()`，这是安全的；动态 fork 出来的任务会被 outer group 自动等待。

但如果某个由 `tg:fork()` 创建出来的 child task 再对这个**同一个 outer group** 调用 `tg:join_all()`，它会把自己也算进等待集合，形成自等待死锁。Verilua 会在非 owner task 调用 `tg:join_all()` 时直接报错。
...
:::warning[只有 task_group owner task 可以调用 `tg:join_any()`]
和 `join_all()` 一样，child task 不应该对包含它自己的 outer group 调用 `join_any()`。这类代码语义含混，并可能形成对自身成员资格的等待。Verilua 会在非 owner task 调用 `tg:join_any()` 时直接报错；需要局部同步时请改用 inner `task_group`。
:::
...
- **child 复用 outer `tg` 来 `tg:fork()`**：可以，outer group 会统一追踪并等待
- **child 复用 outer `tg` 来 `tg:join_all()` / `tg:join_any()`**：不可以，只有创建该 group 的 owner task 可以调用 join；child 应改为创建新的 inner `task_group`

:::tip[何时使用 `task_group` vs `jfork` + `join`]
...
`time` 这一个行为控制机制不需要使用到具体的硬件信号，只需要在任务中使用 `await_time(XXX)` 即可，其中 `XXX` 是指定的时间，单位与仿真器的时间单位相当。完整的时间等待 API（包括 `await_time_ns`、`await_time_us` 等带单位版本）见 [时间等待 API](./await_time.mdx)，例如：
```lua
fork {
    function ()
        print("start task 1")
...
4. `scheduler:get_running_tasks()`
    ...
    :::note[该方法只返回正在运行的任务，不包含已完成或已删除的任务]
    :::
...
- [时间等待 API](./await_time.mdx)：查看 `await_time_ns`、`await_time_us` 等带单位的等待接口。
- [任务同步事件（EventHandle）](./data_structure/event_handle.mdx)：查看任务同步事件句柄的定位说明。
- [信号句柄（CallableHDL）](./data_structure/callable_hdl.mdx#chdl-posedge)：查看 `posedge`、`negedge`、`edge` 等接口的详细行为。
- [时钟驱动策略](../how-to-guides/clock_driving.mdx)：了解多任务等待与不同驱动方式的配合关系。
