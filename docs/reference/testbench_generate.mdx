...
| `--verbose` | 输出详细日志（文件列表、port 列表、regen 原因等）。**默认安静**：只打一行 `generate ...` 或 `up-to-date`；xmake 默认也不再传 `--verbose`，需要时用 `add_values("verilua.tb_gen_flags", "--verbose")`。 |

`--inject-*` 系列的旧名 `--cc` / `--ccs` / `--cco` / `--ccso`（即 `--custom-code*`）仍然生效，但已废弃，新写的配置请使用 `--inject-*` 名称。
...
`tb_top.sv` 包含 DUT 的例化、时钟/复位生成、波形控制接口、DPI 函数声明等。每次重新生成 testbench 时，该文件都会被覆盖，因此不应直接修改，而应通过 `--inject-inner-file` / `--inject-inner-str`（即 `--iif` / `--iis`）等 `--inject-*` 系列选项插入自定义代码。
...
2. 若未指定，则在输入端口中按优先级自动检测（不区分大小写）：精确名 `clk`/`clock`（复位为 `rst`/`reset`）优先于主动低名 `rst_n`/`reset_n` 等，再优先于 `clk_*`/`*_clk`/`i_clk*`、`rst_*`/`*_rst`/`*_rst_n` 等模式匹配。

如果设计中多个端口命中不同优先级，`testbench_gen` 会选择优先级最高的端口；若命中同一优先级，则会报错并列出候选，要求通过 `--clock-signal`（或 `--reset-signal`）显式指定。对于多时钟设计，建议始终显式指定 `--clock-signal`。
