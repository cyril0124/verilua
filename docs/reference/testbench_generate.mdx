...
| `--custom-code <file>` | 将文件内容插入到生成的 testbench 模块内部底部，即 `endmodule` 之前。 |
| `--custom-code-str <string>` | 将字符串内容直接插入到生成的 testbench 模块内部底部，即 `endmodule` 之前。 |
| `--custom-code-outer <file>` | 将文件内容插入到生成的 testbench `module` 声明之前。 |
| `--custom-code-str-outer <string>` | 将字符串内容直接插入到生成的 testbench `module` 声明之前。 |
...
在 Verilua 的默认工作流中，这两个文件会生成到当前 target 的构建目录，而不是源码目录。构建目录的命名和位置可参考 [`cfg.build_dir_name` / `cfg.build_dir_path`](./xmake_params.mdx) 的说明；默认情况下，该目录通常是 `./build/<simulator>/<top module name>`。Verilua 调用 `testbench_gen` 时会自动把这个构建目录传给 `--out-dir`。如果直接手动运行 `testbench_gen`，未指定 `--out-dir` 时则默认输出到当前目录。

```mermaid
flowchart LR
    A[testbench_gen outputs] --> B[tb_top.sv]
    A --> C[others.sv]
    D[custom-code-outer / custom-code-str-outer] --> B
    E[custom-code / custom-code-str] --> B
...
//    use `--custom-code-outer/-cco <file>` or
//        `--custom-code-str-outer/-ccso <string>`.
// -----------------------------------------
...
//    use `--custom-code/-cc <file>` or
//        `--custom-code-str/-ccs <string>`.
// -----------------------------------------
...
| 参数 | 传入形式 | 实际插入位置 | 典型用途 |
| --- | --- | --- | --- |
| `--custom-code-outer <file>` / `-cco` | 文件 | `module <tbtop>(...)` 之前 | 宏定义（如 `define`）、`include`、package import、辅助模块声明 |
| `--custom-code-str-outer <string>` / `-ccso` | 字符串 | `module <tbtop>(...)` 之前 | 很短的宏定义或预处理语句 |
| `--custom-code <file>` / `-cc` | 文件 | testbench 模块内部、`endmodule` 之前 | `initial` / `always` / assertion / monitor 逻辑 |
| `--custom-code-str <string>` / `-ccs` | 字符串 | testbench 模块内部、`endmodule` 之前 | 很短的内联调试逻辑 |

插入位置关系如下：

```verilog
// --custom-code-str-outer / --custom-code-outer inserted here
...
// --custom-code-str / --custom-code inserted here
...
- 需要插入多行代码、单独维护文件、或者包含复杂 `initial` / `always` 块时，优先使用 `--custom-code` 或 `--custom-code-outer`。
- 只需要插入一两行简短宏定义或调试语句时，使用 `--custom-code-str` 或 `--custom-code-str-outer` 更方便。
- `outer` 系列插入的是模块外部代码，适合预处理宏、`include`、package import、辅助 module/function/task 声明；不适合直接写依赖 testbench 内部信号的逻辑。
- `inner` 系列插入的是模块内部代码，可以直接写 `initial`、`always`、assertion 和其他 testbench 逻辑。
- 同一侧的 file 与 string 形式可以同时使用；若同时指定，`-str` 的内容会先插入，文件内容随后插入。
- `--custom-code-str` 和 `--custom-code-str-outer` 本身支持多行字符串；`testbench_gen` 会按原样插入，不会自动重新排版。
...
```lua
add_values("cfg.tb_gen_flags", "--custom-code-outer", "tb_prelude.svh")
```
...
```lua
add_values("cfg.tb_gen_flags", "--custom-code-str-outer", "`define TB_TIMEOUT 10000")
```
...
```lua
add_values("cfg.tb_gen_flags", "--custom-code", "tb_hooks.sv")
```
...
```lua
add_values("cfg.tb_gen_flags", "--custom-code-str", 'initial $display("[tb] ready");')
```

适用场景：只想临时加一条很短的调试输出。

示例 5：在 `xmake.lua` 中传入多行 `--custom-code-str`
...
示例 6：在 `xmake.lua` 中传入多行 `--custom-code-str-outer`
...
示例 7：直接命令行传入多行 `--custom-code-str`
...
示例 8：直接命令行传入多行 `--custom-code-str-outer`
...
testbench_gen \
  --top MinimalTop \
  --out-dir ./build/verilator/MinimalTop \
  --custom-code-str-outer $'`define TB_DIRECT_OUTER 1\n`define TB_DIRECT_OUTER_2 2' \
  minimal_top.sv
```
...
- 如果直接命令行调用 `testbench_gen`，`--custom-code-str` 和 `--custom-code-str-outer` 需要根据所用 shell 正确处理引号、反引号和换行；复杂内容通常更建议使用 file 形式。
- 在 `xmake.lua` 中，`cfg.tb_gen_flags` 现在可以正确透传包含空格和换行的 `--custom-code-str` / `--custom-code-str-outer` 参数。
- 如果内容较长、需要复用、或者需要避免 shell 引号问题，依然更推荐 `--custom-code <file>` 和 `--custom-code-outer <file>`。
...
在 `xmake.lua` 中，通过 `cfg.tb_gen_flags` 传入 include 路径：

```lua
add_values("cfg.tb_gen_flags", "-I", path.join(os.scriptdir(), "include"))
```
...
- [xmake 参数说明](./xmake_params.mdx)：查看 `cfg.tb_gen_flags`、构建目录和仿真器参数的完整说明。
