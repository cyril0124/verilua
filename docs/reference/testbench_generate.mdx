...
```lua
add_values("verilua.tb_gen_flags", "--custom-code-outer", "tb_prelude.svh")
```
...
```lua
add_values("verilua.tb_gen_flags", "--custom-code-str-outer", "`define TB_TIMEOUT 10000")
```
...
```lua
add_values("verilua.tb_gen_flags", "--custom-code", "tb_hooks.sv")
```
...
```lua
add_values("verilua.tb_gen_flags", "--custom-code-str", 'initial $display("[tb] ready");')
```
...
- 在 `xmake.lua` 中，`verilua.tb_gen_flags` 现在可以正确透传包含空格和换行的 `--custom-code-str` / `--custom-code-str-outer` 参数。
