...
- `set_values("verilua.user_cfg", ...)` 只是用来指定用户配置文件路径，运行期不会自动出现 `cfg.user_cfg` 这个字段；该文件返回的字段会直接合并到全局 `cfg` 中
:::
...
指定一个或多个 Lua 配置文件，并将它们返回的 table 合并到运行期全局变量 `cfg` 中。
...
1. 按声明顺序逐个加载（上例为 `common/cfg.lua` → `case/cfg.lua` → `extra/cfg.lua`），后加载的文件覆盖先前的同名字段。`srcs` 和 `deps` 除外，这两个字段是追加。
2. 每个文件按完整路径加载，不同目录下都叫 `cfg.lua` 也不会互相覆盖。
3. 加载某个配置文件时，`base_path` 中的路径先于当前配置目录搜索，其中包括 Verilua 自身与 `deps` 的路径。全部加载完后，所有配置目录一起保留在 `package.path` 中。

:::warning[同名模块]
Lua 的模块缓存（`package.loaded`）按**模块名**索引，不区分来源目录。两个配置目录下都有 `helper.lua`，且两个配置文件都 `require "helper"` 时，第二次 `require` 拿到的仍然是第一次加载的模块。共享模块请用唯一名字，例如 `common_helper.lua`。

配置文件内部对 `package.path` 的修改也不会保留到加载结束之后。需要额外搜索路径时，用 `add_files` 或把模块放在配置文件同目录。
:::
...
- [xmake 参数说明](./xmake_params.mdx)：查看 `set_values("verilua.xxx", ...)` 的构建期配置入口。
