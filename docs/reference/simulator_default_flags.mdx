...
- **可开关切换**：Verilator 的 `-O3`↔`-O0`、`--no-timing`↔`--timing`；寄存器初始化通过 [`vcs_no_initreg` / `xcelium_no_initreg`](#关闭寄存器初始化) 关闭；`--public-flat-rw` 通过 [`verilua.verilator_no_public_flat_rw`](./xmake_params.mdx) 显式关闭（与是否存在 `.vlt` / `verilua.verilator_config` 无关）。
...
| `--public-flat-rw` | 允许通过 VPI 读写所有信号（默认注入；`set_values("verilua.verilator_no_public_flat_rw", "1")` 可关闭，再靠 `.vlt` / `verilua.verilator_config` 精细 public） |
