...
- **HVL 场景**：设置为实际的顶层 DUT 名称（如 `Design`、`Counter`），Verilua 会据此生成 testbench 并自动将运行期 `cfg.top` 设为 testbench 顶层名。
- **WAL 场景**：根据波形文件的层次结构设置，详见下方说明

例如在[这个例子](../getting-started/simple_hvl_example.mdx) 中我们顶层 DUT 模块的名称是 `Design`，因此可以这么设置：`set_values("verilua.top", "Design")`。

### 指定 Lua 入口脚本

对应配置：`set_values("verilua.lua_main", <lua main file>)`

设置仿真时候执行的 lua 主脚本文件，对于 HVL/WAL 场景，这个参数必须设置，例如在[这个例子](../getting-started/simple_hvl_example.mdx) 中我们的 lua 主脚本文件是 `LuaMain.lua`，因此可以这么设置：`set_values("verilua.lua_main", "LuaMain.lua")`。
...
- `set_values(<key>, <value>)` 对 `<key>` 进行单次赋值，调用多次 `set_values` 时，会覆盖之前的值。需要一次设置多个值时，也可以传入多个 `<value>`；
...
```mermaid
flowchart LR
    A[xmake target] --> B[add_rules / add_toolchains / add_files]
    B --> C[verilua.tb_gen_flags]
    C --> D[testbench_gen phase]
    D --> E[Generated tb_top.sv]
    E --> F[sim.flags compile phase]
    F --> G[Simulator binary or backend target]
    H[sim.run_prefix] --> G
    G --> I[sim.run_flags]
    I --> J[Final run command]
```
...
### 内联 Verilator control file

对应配置：`set_values("verilua.verilator_config", <vlt content>)` / `add_values("verilua.verilator_config", <vlt content>)`

把 Verilator control file 内容直接写在 `xmake.lua` 里，无需单独维护文件，也不用 `add_files("*.vlt")`。构建时会写入当前 target 构建目录下的 `verilua_generated.vlt`。语法与可用指令见 [Verilator Control Files](https://verilator.org/guide/latest/control.html#verilator-control-files)。

- 仅 `verilator` 后端生效；其它仿真器忽略
- **只叠加指令**，不会隐式关掉默认 `--public-flat-rw`
- 内容里若没有 `verilator_config`，会自动包一层 `` `verilator_config ``
- 可与 `add_files("*.vlt")` 同时使用；两者都会传给 Verilator
- 内容（或任一 `.vlt`）含 `forceable` 时，构建期要求 Verilator ≥ 5.050（VPI force/release）
...
target("test", function()
    ...
    -- forceable only; default --public-flat-rw still applies
    set_values("verilua.verilator_config", [[
forceable -module "top" -var "i_ready"
]])
end)
```
...
对应配置：`set_values("verilua.verilator_no_public_flat_rw", "1")`

默认始终注入 `--public-flat-rw`（Verilua VPI 需要）。若要用 `.vlt` / `verilua.verilator_config` 做精细 `public_flat_*` 以提升性能，显式关闭：

```lua title="xmake.lua"
set_values("verilua.verilator_no_public_flat_rw", "1")
set_values("verilua.verilator_config", [[
public_flat_rw -module "tb_top" -var "*"
public_flat_rw -module "top" -var "*"
]])
-- 或 add_files("config.vlt")
```

### Verilator C++ 优化级别（OPT_FAST / OPT_SLOW）

对应配置：

- `set_values("verilua.verilator_opt_fast", <flags>)`：快路径（每周期执行的 model 代码），默认 `-O3 -march=native`
- `set_values("verilua.verilator_opt_slow", <flags>)`：慢路径（初始化/收尾），默认 `-O0`

传给 Verilator 生成的 `Makefile` 的 `OPT_FAST` / `OPT_SLOW`，**不是** `verilator` CLI 参数。日常开发想加快编译可把 fast 降为 `-O0`：
...
设置构建目录的名称，如果不设置，默认为 [`set_values("verilua.top", <top module name>)`](#cfg-top) 的值。
...
对应配置：`set_values("verilua.user_cfg", <user cfg file>, ...)` 或 `add_values("verilua.user_cfg", <user cfg file>)`

用于指定一个或多个用户自定义 Lua 配置文件。

这些 Lua 文件都需要返回一个 table；在运行期，table 里的字段会被合并到全局变量 `cfg` 中。

多个文件按声明顺序依次加载合并，后加载的文件覆盖先加载的同名字段。`srcs` 和 `deps` 除外，这两个字段会追加：

```lua
set_values("verilua.user_cfg", "common/cfg.lua", "case/cfg.lua")
add_values("verilua.user_cfg", "extra/cfg.lua") -- 追加在最后，优先级最高
```

每个配置文件按完整路径加载，即使多个配置文件同名（都叫 `cfg.lua`）也不会互相覆盖。配置文件内可以 `require` 同目录下的模块。合并顺序与同名模块的限制见 [`global_configuration`](./global_configuration.mdx#cfg-user-cfg)。

:::warning[常见误解]
这里的 `verilua.user_cfg` 只是 xmake 配置项名，不代表运行期存在 `cfg.user_cfg` 这个字段。
:::

完整的运行期行为说明见 [`global_configuration`](./global_configuration.mdx#cfg-user-cfg)。

### 限制最低 Verilua 版本

对应配置：`set_values("verilua.require_version", "<version constraint>")`
...
- [仿真后端默认参数](./simulator_default_flags.mdx)：了解 Verilua 为每个后端自动注入的编译/运行参数（寄存器初始化、timescale 等）。
- [编写 xmake.lua](../how-to-guides/write_xmake_lua.mdx)：查看这些参数在完整工程中的组织方式。
- [HVL 示例](../getting-started/simple_hvl_example.mdx)：对照最小示例理解 `verilua.top` 和 `verilua.lua_main` 的基本用法。
- [Testbench 自动生成流程](./testbench_generate.mdx)：查看 `verilua.tb_gen_flags` 等参数如何影响生成结果。
- [全局配置信息](./global_configuration.mdx)：了解构建期 `verilua.xxx` 与运行期全局 `cfg` 的关系。
