...
SVBuilder 是 Verilua 提供的 SystemVerilog 生成工具，支持 SVA（断言/覆盖）和 covergroup（功能覆盖率）。它的核心价值在于**复用 Verilua testbench 中已有的信号 handle**，结合 Lua 的编程能力（循环、条件、模板），批量生成可嵌入 RTL 的 SV 文本，由仿真器原生引擎执行。
