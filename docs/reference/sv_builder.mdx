...
- `"covergroup"` → 返回该 covergroup 的实例名字符串（`_GEN_<name>_inst`，见下文「显式采样」）
...
`add "covergroup"` 返回该 covergroup 的实例名字符串（`_GEN_<name>_inst`），采样调用点用它拼出来即可，不必自己记实例名格式。

采样调用点要自己放进 SV。最方便是写进 `add "raw"`，把实例名通过 `envs` 交给模板引擎渲染：`raw` 里的内容统一输出在 covergroup 之前（不管 `add` 的顺序），而 module 作用域内只要求声明与使用同属一个 module，所以这条前向引用合法：
...
local cg_inst = ctx:add "covergroup" {
    name = "cg_axi_cmd",
...
ctx:add "raw" {
    ...
    envs = { cg_inst = cg_inst, args = "burst, len" },
}
```
