...
SVBuilder 是 Verilua 提供的 SystemVerilog 生成工具，用于生成 SVA 断言/cover 语句和 covergroup（功能覆盖率）。它复用 Verilua testbench 中已有的信号 handle，结合 Lua 的循环、条件、模板能力，批量生成可嵌入 RTL 的 SV 文本，由仿真器原生引擎执行。
...
柯里化调用，`typ` 为 `"sequence"` | `"property"` | `"cover"` | `"assert"` | `"covergroup"` | `"raw"`。
...
ctx:add "covergroup" { name = "cg1", expr = "..." }
ctx:add "raw"        { name = "helper", expr = "logic sample_en;" }
```
...
| `expr` | string | ✓ | SVA/SV 表达式，支持 `$(var)` 模板语法 |
| `envs` | table | | 模板变量表，key→value 映射 |
| `formal_args` | string | | 仅 `sequence`/`property` 有效：形参列表，如 `"logic req, logic ack"`，直接插入括号内 |
| `cov_type` | string | | 仅 cover 有效：`"property"`（默认）或 `"sequence"` |
| `sample_event` | string | | 仅 covergroup 有效：覆盖默认采样事件 |

**返回值：**

- `"sequence"` → 返回 `Sequence` handle（可通过 `$(seq:name)` 引用）
- `"property"` → 返回 `Property` handle（可通过 `$(prop:name)` 引用）
- `"covergroup"` → 返回 `Covergroup` handle，`.name` 为 covergroup 类型名，`.inst_name` 为生成的实例名（`_GEN_<name>_inst`）；可通过 `$(cov:name)` 在 `add "raw"` 的 expr 里引用（渲染为 `inst_name`）
- `"cover"` / `"assert"` / `"raw"` → 返回 `nil`
...
ctx:default_clocking("top.dut.clk", "posedge")  -- 无 handle 时直接给层次路径
```

- `signal`：`CallableHDL` / `ProxyTableHandle`（1-bit），或层次路径字符串（如 `"top.dut.clk"`）
...
### `ctx:set_coverage_report(enable)`

启用或禁用 covergroup 的 `final` coverage 报告（默认开启）。
...
## 命名空间引用：`$(seq:name)` / `$(prop:name)` / `$(cov:name)`
...
### 带 `formal_args` 的 sequence 实参传递

如果 sequence 声明了 `formal_args`，在 `expr` 里引用时需要在 `$(seq:name)` 后面直接跟括号传入实参。`$(seq:name)` 渲染为 sequence 名称，括号和其中的模板表达式由引擎继续展开：
...
-- 在引用处传入实参：$(seq:handshake) 渲染为 "handshake"，
-- ($(req), $(ack)) 继续由模板引擎展开
ctx:add "assert" {
    name = "chk",
    expr = "$(seq:handshake)($(req), $(ack)) |-> !$(err)",
    envs = { req = dut.req, ack = dut.ack, err = dut.err },
}
-- 生成: handshake(top.dut.req, top.dut.ack) |-> !top.dut.err
```

如果有 `formal_args` 的 sequence 被裸 `$(seq:name)` 引用（不传实参），SV 是非法的，sv_lint 会在 `add` 阶段报出 slang 错误：

```
[SVBuilder] lint error in 'chk'
error: too few arguments for 'handshake'; expected 2 but 0 were provided
```

这样设计的好处：
- 引用点一眼能看出类型（sequence 还是 property）
- sequence/property 不会和普通 env 变量命名冲突
- 有形参的 sequence 缺实参时，lint 在定义阶段即报错，不会等到仿真器运行时才发现
...
```
[SVBuilder] lint error in 'chk_handshake'
source: /project/tb/checks.lua:137

dump: /tmp/sv_builder_lint_lua_XXXXXX.sv

/tmp/sv_builder_lint_lua_XXXXXX.sv:4:8: error: sequence range minimum '5' is greater than specified maximum '2'
    ##[5:2] top.dut.ack
       ^ ~
/tmp/sv_builder_lint_lua_XXXXXX.sv:2:10: note: while expanding sequence 'handshake'
...
- `source`：调用 `ctx:add(...)` 的 Lua 文件和行号
- `dump`：送进 slang 的生成 SV 文件；后续诊断中的行号、列号和 caret 都对应这个文件

lint 失败时，会把送进 slang 的那段 SV 写到 `/tmp/sv_builder_lint_<随机>.sv`。报错里 `dump:` 后面就是路径，诊断里的文件名也指向它，行号和文件内容对齐。失败的那条不会进入 `generate()`。
...
1. 所有 `raw` preamble（typedef / function / logic / always 等）
2. `default clocking` 声明（如果设置了）
3. 所有 `sequence` 定义
4. 所有 `property` 定义
5. 所有 `cover` / `assert` 语句（带 `// i/n` 编号注释）
6. 所有 `covergroup` 定义与实例化
7. `final` coverage 报告（若启用）

`raw` 排在 `default clocking` 之前，这样 preamble 里声明的 bare `clock` / `reset` 能被 clocking 与后续 lint 解析（与注入 `tb_top` 后的常见写法一致）。
...
用 SVBuilder，信号路径从 testbench 已有的 handle 自动提取。路径变了（比如模块重命名）只需改 testbench 一处，生成的 SVA 随之更新：
...
SVBuilder 只需一个循环。新增 channel 在 `channels` 表里加一行即可，断言逻辑只写一份，不会漏改某个 channel：
...
SVBuilder 用模板循环生成，beat 数量变化时只改循环边界，信号 handle 和循环变量可以混用：
...
final begin
    $display("[COVERAGE] cg_axi_cmd: %.2f%%", _GEN_cg_axi_cmd_inst.get_coverage());
end
```
...
| `sample_event` | string | ✗ | 覆盖默认采样事件（默认复用 `default_clocking`）；也支持 `"with function sample(...)"` 声明显式采样形参 |
...
1. **外层包裹**：`covergroup name @(event); ... endgroup`
2. **实例化**：`name _GEN_name_inst = new;`
3. **Coverage 报告**：`final begin ... $display ... end`
...
### 显式采样（with function sample）

`sample_event` 写成 `with function sample(...)` 时，covergroup 头不带 `@()`，采样由外部显式调用 `sample()` 决定。形参在该 covergroup 体内直接可见，反过来说：这些形参在 `add "assert"` / `add "cover"` 生成的上行 SVA 里不可见，`envs` 也影响不到它们。

`add "covergroup"` 返回一个 `Covergroup` handle，`.inst_name` 字段持有生成的实例名。采样调用点要自己放进 SV，最方便是写进 `add "raw"`，有两种方式引用实例名：

- **`$(cov:name)`**：直接在 raw expr 里通过 `cov:` 命名空间引用（推荐）
- **`.inst_name`**：把 handle 的 `.inst_name` 字段放进 `envs`，通过普通模板变量引用

`raw` 里的内容统一输出在 covergroup 之前，而 module 作用域内只要求声明与使用同属一个 module，所以这条前向引用合法：
...
local cov = ctx:add "covergroup" {
    ...
    expr = [[    cp_burst: coverpoint burst { bins incr = {4'd1}; }
    cp_len: coverpoint len     { bins short = {[0:15]}; bins long = {[16:255]}; }]],
}

-- 方式一：$(cov:cg_axi_cmd) 直接在 expr 里引用（推荐）
ctx:add "raw" {
    name = "cg_axi_sample_driver",
    expr = [[always @(posedge clk) begin
    $(cov:cg_axi_cmd).sample(burst, len);
end]],
}

-- 方式二：把 .inst_name 放进 envs，作为普通模板变量
-- ctx:add "raw" {
--     name = "cg_axi_sample_driver",
--     expr = [[always @(posedge clk) begin
--     $(cg_inst).sample($(args));
...
例子里第一个 `raw` 只是让示例自带声明；实际工程里 `clk` / `burst` / `len` 由 `testbench_gen` 生成的 `tb_top` 提供，只要能解析，`sv_lint` 就不会在 `add` 阶段报未声明标识符。生成结果：
...
always @(posedge clk) begin
    _GEN_cg_axi_cmd_inst.sample(burst, len);
end

covergroup cg_axi_cmd with function sample(bit [3:0] burst, bit [7:0] len);
    cp_burst: coverpoint burst { bins incr = {4'd1}; }
    cp_len: coverpoint len     { bins short = {[0:15]}; bins long = {[16:255]}; }
endgroup
cg_axi_cmd _GEN_cg_axi_cmd_inst = new;
...
sv_lint 同样检查 covergroup，能在定义时捕获：
...
功能覆盖经常需要 covergroup 之外的辅助 SV：`typedef`、function、中间 `logic`、`always` 采样状态机。用 `add "raw"` 把这些内容放进 preamble（在 `default clocking` / sequence/property/covergroup 之前），后续 covergroup 可直接引用这些信号；lint 上下文也会包含 preamble。

```lua
-- bare clock/reset：先在 raw 里声明，再 default_clocking（输出/lint 顺序固定为 preamble → clocking）
ctx:add "raw" {
    name = "fcov_helper",
    expr = [[
logic clock;
...
always @(posedge clock or posedge reset) begin
    if (reset) sample_en <= 1'b0;
        ...
        sample_en <= req_fire;
        size      <= req_size;
        be_class  <= be_class_fn(req_be, req_size);
    end
end
]],
}
ctx:default_clocking("clock", "posedge") -- 也可用 dut.clk handle / 层次路径字符串

ctx:add "covergroup" {
    name = "cg_req",
    expr = [[
    cp_size: coverpoint size iff (sample_en) {
        bins B1 = {3'd0};
        bins B2 = {3'd1};
    }
    cp_be: coverpoint be_class iff (sample_en) {
        bins Zero    = {2'd0};
        bins Partial = {2'd1};
        bins Full    = {2'd2};
    }
...
- `expr` 原样保留换行（不会做 SVA 那套空白压缩）
- 支持 `$(var)` 模板与 `envs`
- 参与 `sv_lint`：raw 自身语法错误会立刻报；后续语句 lint 时能看到 preamble 里声明的标识符
- 输出顺序固定为 `raw` → `default clocking` → …，与 `add` / `default_clocking` 的调用先后无关
