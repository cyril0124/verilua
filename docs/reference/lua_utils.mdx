...
`LuaUtils` 是 Verilua 提供的通用工具模块，包含数值转换、位操作、字符串处理、枚举定义、随机数生成、环境变量读取、矩阵调用等常用功能。它是验证代码中使用频率最高的基础工具集之一。
...
| 函数 | 描述 |
| --- | --- |
| `matrix_call(func_table)` | 多维函数组合的笛卡尔积调用 |
...
读取环境变量并转换为指定类型。如果环境变量未设置，返回 `default` 并打印警告日志。
...
#### `matrix_call(func_table)`

对多维函数列表执行笛卡尔积组合调用。每个维度包含若干函数选项，`matrix_call` 会遍历所有维度的所有组合并依次执行。

支持四种函数块类型：
- **单函数**：直接传入 `function`
- **顺序函数组**：传入 `{func1, func2, ...}`，按顺序执行
- **带参数的函数**：传入 `{func = f, args = {...}}`
- **带多组参数的函数**：传入 `{func = f, multi_args = {{...}, {...}}}`
...
utils.matrix_call {
    {
        { func = function(x) print("x =", x) end, args = {10} },
        { func = function(x) print("x =", x) end, args = {20} },
    },
}
-- 输出：
-- x = 10
