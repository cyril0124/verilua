...
1. `sim.dump_wave(trace_file_path)`
    ...
    - **说明**：
        ...
        - 对于 Xcelium，若未指定文件名，将使用 `waves.shm`；若指定，则根据文件扩展名决定波形格式（`.vcd` 为 VCD，`.fsdb` 为 FSDB）。FSDB 格式需要 `PATH` 中可找到 `verdi`，Verilua 会从其真实路径推导安装目录并启用相应宏。
    - **示例**：
        ```lua
...
- [时间等待 API](./await_time.mdx)：结合 `sim.get_sim_time()` 理解时间单位和等待行为。
- [时钟驱动策略](../how-to-guides/clock_driving.mdx)：查看波形控制与时钟驱动配置的关系。
- [WAL 示例](../getting-started/simple_wal_example.mdx)：查看 `sim.dump_wave()` 在生成波形时的实际用法。
