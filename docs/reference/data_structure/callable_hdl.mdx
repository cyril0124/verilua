...
11. `<chdl>:set_force(value)`
    ...
    :::note[Verilator 需要 ≥ 5.046 且 forceable]
    在 Verilator 上使用 `set_force` / `set_release` 时：
    ...
    2. **版本**：需要 Verilator ≥ 5.046。xmake rule 在 control file 里出现 `forceable` 时会在**构建期**检查版本；过低会直接 build fail，避免 force 被当成普通写而无声失败。
