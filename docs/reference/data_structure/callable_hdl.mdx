...
这种方式在 Verilua 中被称为 **S**tring **L**iteral **C**onstructor **P**attern（SLCP），使用 SLCP 的好处在于可以不用像 `class` 构建那样提前使用 `require` 将 `LuaCallableHDL` 加载到代码中，而是在使用时根据后缀的方法名（`:chdl()`）来直接创建 `chdl`。除了 `chdl` 之外，其他的数据结构同样也支持使用 SLCP 的方式来创建，详见 [字符串字面量构造模式（SLCP）](../slcp.mdx)。

### 信号名与接口名重名时

`dut.path.to.signal` 这种写法把信号名当字段查，而 `set`、`set_unchecked`、`randomize`、`force`、`release`、`freeze`、`get`、`name`、`dump` 这些接口名优先命中。RTL 里真有同名信号时，`dut` 路径拿到的是接口函数：

```lua
type(dut.u_top.set) -- "function"，不是信号
```

用字符串路径构造 `chdl`，信号名只是路径的一部分，不参与名字解析：

```lua
local sig = ("tb_top.u_top.set"):chdl()

-- 已经持有父节点时，用 get_local_path() 拼出完整路径
local u_top = dut.u_top
local sig = (u_top:get_local_path() .. ".set"):chdl()
```
...
[Bundle](./bundle.mdx#信号名与-bundle-接口重名) 上的重名会在构造时报错，处理方式一样。如果想保留结构化的访问方式，可以用 [AliasBundle](./bundle.mdx#bundle-vs-aliasbundle-选型) 把它映射成不重名的别名。
...
3. `<chdl>:get_bitvec()`
   ...
   返回一个 `BitVec`，关于 `BitVec` 可以查看 [位向量（BitVec）](../bitvec.mdx) 的文档。
...
写入默认是延迟的，写完马上 `get()` 读到的还是旧值。每个方法都有立即生效的 `_imm` 版本，区别见[立即赋值和普通赋值的区别](#set-and-set-imm)。

| API | 作用 | 立即版本 |
|---|---|---|
| `set(v)` | 常规写入，根据位宽校验 `v` 的类型与大小 | `set_imm` |
| `.value = v` | 属性写法，等价于 `set(v)` | `.value_imm = v` |
| `set_unchecked(v)` | 跳过校验的写入，用于逐拍写的热点路径 | `set_imm_unchecked` |
| `set_bits(s, e, v)` | 只改 `[e:s]` 这段位，其余位保持不变，位段宽度上限 64 bit | `set_bits_imm` |
| `set_bits_hex_str(s, e, hex_str)` | 同上，但值用十六进制字符串给，位段可以超过 64 bit | `set_bits_imm_hex_str` |
| `set_str(str)` | 字符串写入，由前缀定进制：`0x` 走十六进制，`0b` 走二进制，其余按十进制 | `set_imm_str` |
| `set_hex_str(str)` | 进制已知时直接指定，另有 `set_bin_str` / `set_dec_str` | `set_imm_hex_str` 等 |
| `randomize()` | 写入一个随机值 | `randomize_imm` |
| `force(v)` | 强制驱动，同 SystemVerilog 的 `force`，值上限 64 bit（或按 beat 给一个数组） | `force_imm` |
| `force_hex_str(str)` | 用十六进制字符串强制驱动，值可以超过 64 bit | `force_imm_hex_str` |
| `release()` | 解除 `force` / `freeze` | `release_imm` |
| `freeze()` | 把信号钉在当前值，用 `release()` 解除 | `freeze_imm` |
| `set_cached(v)` | 值和上次写入相同就跳过本次写，用 `reset_set_cached()` 清缓存 | `set_imm_cached` |

数组信号额外有下标和整体写入：

| API | 作用 | 立即版本 |
|---|---|---|
| `arr[i]` | 取第 `i` 个元素的句柄，从 0 数，同一下标返回同一个句柄 | 无 |
| `set_all(values)` | 按 `values` 依次写入整个数组 | `set_all_imm` |
| `set_all_unchecked(values)` | 跳过校验的整数组写入 | `set_all_imm_unchecked` |

```lua
local arr = dut.u_top.array_signal:chdl()
arr[0]:set(0x11)
arr[0].value = 0x22
arr:set_all { 0x11, 0x22, 0x33, 0x44 }
```
...
1. `<chdl>:set(value)`
   ...
   <Tabs>
     ...
     <TabItem value="double-multi" label="Double / Multi">
       ...
       根据 `value` 的类型自动选择赋值路径：

       - 如果 `value` 是一个 **table**，则按多拍赋值。table 的大小需要和信号的 beat 数相同，否则会报错。table 的数值以 `<LSB> ~ <MSB>` 的顺序排列。
       ...
       - 如果 `value` 是一个 **number** 或 `uint64_t` 类型的 `cdata`，则按标量赋值。number 赋值信号的低 32 bit，`uint64_t` 赋值信号的低 64 bit，高于 64 bit 的位置将会被赋值为 0。
         ...
         ```
          ...
          如果你刚从标准 Lua 切换过来，还不熟悉 `ULL` 和 `cdata`，可以先看[LuaJIT 与标准 Lua 的常见差异](../../getting-started/luajit_vs_standard_lua.mdx#data-type-differences)。
...
8. `<chdl>:set_bits(s, e, v)`

   设置信号的值 `v`，并且只在 `s` 到 `e` 之间的位宽上赋值。`v` 可以是 Lua number 类型的数值，也可以是一个 `uint64_t` 类型的 `cdata`，因此位段宽度（`e - s + 1`）不能超过 64 bit。

   ```lua
   local signal = dut.value:chdl()

   local value = 0x123
   signal:set_bits(0, 7, value)
   ```

9. `<chdl>:set_bits_hex_str(s, e, hex_str)`

   和 `set_bits` 一样只写 `s` 到 `e` 这段位，但值用十六进制字符串给，所以位段可以超过 64 bit：

   ```lua
   local wide = ("tb_top.u_top.wide_signal"):chdl() -- 128 bit
   wide:set_bits_hex_str(0, 127, "1234567890abcdef1234567890abcdef")
   ```

10. `<chdl>:force(value)`

    强制赋值（与 `release` 配合使用），与 SystemVerilog 中的 `force` 关键字相同。除了 `force` 这个属性上的区别之外，其他和 `set` 一样。

    :::note[Verilator 需要 ≥ 5.050 且 forceable]
    在 Verilator 上使用 `force` / `release` 时：

    1. **forceable**：目标信号必须标记为 forceable，例如通过 [`verilua.verilator_config`](../xmake_params.mdx) 或 `.vlt`：
    ...
    2. **版本**：需要 Verilator ≥ 5.050。xmake rule 在 control file 里出现 `forceable` 时会在**构建期**检查版本；过低会直接 build fail，避免 force 被当成普通写而无声失败。

    详见 [Verilator Control Files](https://verilator.org/guide/latest/control.html#verilator-control-files)。
    :::
...
11. `<chdl>:release()`
    ...
    释放赋值（与 `force` 配合使用），与 SystemVerilog 中的 `release` 关键字相同。对于使用了 `force` 的信号，需要使用 `release` 来释放赋值，否则会导致信号的值不会更新。

12. `<chdl>:set_imm(value)`
    ...
    **代码示例：**
    ...
    await_rd()
    assert(signal:get() == 0x123)   -- ReadOnly 阶段可见；此阶段禁止 HDL 写操作
    ```

    `set_imm()` 立即写入，适合需要马上读回的场景：
...
13. `set_imm_*` 立即赋值变体
    ...
    `set_imm_unchecked`、`set_imm_str`、`set_imm_hex_str`、`set_imm_bin_str`、`set_imm_dec_str`、`randomize_imm`、`set_bits_imm`、`set_bits_imm_hex_str`、`force_imm`、`release_imm`、`freeze_imm`、`set_imm_cached`

14. `<chdl>:freeze()`

    冻结当前的信号值，在后续的仿真中，该信号的值将保持不变。需要调用 `<chdl>:release()` 来取消冻结。

15. `<chdl>:set_cached(value)`

    带缓存的赋值：`value` 与上一次通过 `set_cached` 写入的值相同时直接返回，不产生 VPI 调用。适合每拍都要驱动、但取值很少变化的信号。

    ```lua
    local signal = dut.value:chdl()

    signal:set_cached(1) -- 实际写入
    signal:set_cached(1) -- 跳过

    signal:set(2)               -- 绕过缓存写入，缓存仍记录为 1
    signal:reset_set_cached()   -- 清空缓存，下一次 set_cached 必定写入
    ```

    :::warning

    缓存只跟踪 `set_cached` / `set_imm_cached` 自己写入的值。如果信号还被 `set`、`force` 等其他方式改动，需要调用 `reset_set_cached()` 使缓存失效。
...
3. `<chdl>:posedge_until(max_limit, func)`
   ...
   在每一轮检查中，Verilua 会先调用一次 `func`；若返回 `true` 则停止并返回 `true`，若返回 `false` 才会继续等待下一个时钟**上升沿**并进入下一轮。若达到 `max_limit` 轮后 `func` 仍未返回 `true`，则返回 `false`。
...
4. `<chdl>:negedge_until(max_limit, func)`
   ...
   和 `<chdl>:posedge_until(max_limit, func)` 类似，但是在等待信号的**下降沿**到来。调用顺序也相同：会先立即调用一次 `func`，只有返回 `false` 时才会继续等待下一个 `negedge`。
...
- [位向量（BitVec）](../bitvec.mdx)：查看 `get_bitvec()` 返回值和大位宽数据操作方式。
