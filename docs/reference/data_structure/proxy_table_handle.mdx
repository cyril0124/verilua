...
11. `<dut>:get_str(fmt)`
    ...
    作用类似 [`<chdl>:get_str(fmt)`](./callable_hdl.mdx#chdl-get_str)。

12. `<dut>:get_hex_str()`

    作用类似 [`<chdl>:get_hex_str()`](./callable_hdl.mdx#chdl-get_hex_str)。

13. `<dut>:set_str(str)`

    作用类似 [`<chdl>:set_str(str)`](./callable_hdl.mdx#chdl-set_str)。

14. `<dut>:set_hex_str(str)`

    作用类似 [`<chdl>:set_hex_str(str)`](./callable_hdl.mdx#chdl-set_hex_str)。
...
18. `<dut>:posedge_until(max_limit, func)`
    ...
    作用类似 [`<chdl>:posedge_until(max_limit, func)`](./callable_hdl.mdx#chdl-posedge_until)。

19. `<dut>:negedge_until(max_limit, func)`

    作用类似 [`<chdl>:negedge_until(max_limit, func)`](./callable_hdl.mdx#chdl-negedge_until)。
...
20. `<dut>:chdl()`
    ...
    [这里](./callable_hdl.mdx#dut-flex_create_chdl)介绍了有关使用 `<dut>:chdl()` 创建 `chdl` 的技巧。
...
22. `<dut>:dump()` / `<dut>:dump_str()`
    ...
    作用类似 [`<chdl>:dump()`](./callable_hdl.mdx#chdl-dump) / [`<chdl>:dump_str()`](./callable_hdl.mdx#chdl-dump_str)。
