...
`dut` 上的写入和 `CallableHDL` 一样：`set`、`set_imm`、`force`、`release`、`randomize`、`freeze`。
数组也是 `dut.arr[0]`，从 0 数。

```lua
dut.arr[0]:set(1)
dut.arr[0].value = 1
local elem = dut.arr[0]:chdl()
```
...
3. `<dut>:set_imm(<value>)`
   ...
   立即赋值版本的 `set`，除了立即赋值之外，数值处理规则与 `<dut>:set(<value>)` 相同。详见 [立即赋值和普通赋值的区别](./callable_hdl.mdx#set-and-set-imm)。
...
5. `<dut>:force(<value>)`
   ...
   强制赋值，等同 SystemVerilog `force`，需配合 `release` 使用。`<value>` 只能是 Lua number。

   位宽超过 64 bit 的信号用 `<dut>:force_hex_str(str)`（立即版本 `force_imm_hex_str`），十六进制字符串不带前缀：

   ```lua
   dut.u_top.wide_data:force_hex_str("7000000000001dead")
   ```

6. `<dut>:release()`

   释放 `force` 的强制赋值（等同 SystemVerilog `release`）。未调用时信号值将保持不变。

   对应的立即版本为 `<dut>:release_imm()`，用于释放 `force_imm` 的立即强制赋值。
...
20. `<dut>:chdl()`
    ...
    基于 `dut` 当前的 hierarchy path 返回一个新的 `chdl`。`ProxyTableHandle` 内部会复用句柄来实现部分读/检查代理方法，但 `:chdl()` 本身始终返回独立的 `CallableHDL`，以避免可变内部状态在不同查找之间泄漏，例如：
...
24. `<dut>:freeze()`
    ...
    冻结当前的信号值，在后续的仿真中，该信号的值将保持不变。需要调用 `<dut>:release()` 来取消冻结。
...
## 信号名与 ProxyTableHandle 接口重名

上表中的接口名（`set`、`get`、`force`、`release`、`freeze`、`randomize`、`chdl`、`hdl`、`name`、`dump`、`is`、`expect*` 等）优先于同名子节点，所以 RTL 里若有信号叫 `force`，`dut.u_top.force` 拿到的是接口函数而不是信号：

```lua
type(dut.u_top.force) -- "function"，信号被接口遮蔽
```

取这类信号要用字符串路径构造 `chdl`，写法见 [信号名与接口名重名时](./callable_hdl.mdx#信号名与接口名重名时)。
