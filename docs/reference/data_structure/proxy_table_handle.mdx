...
5. `<dut>:set_force(<value>)`
   ...
   立即强制赋值，等同 SystemVerilog `force`，调用返回后即可读取强制值；需配合 `set_release` 使用。`<value>` 只能是 Lua number。

6. `<dut>:set_release()`

   立即释放 `set_force` / `set_freeze` 建立的强制状态（等同 SystemVerilog `release`）。释放不会丢弃同一信号尚未提交的普通 `set()`。

7. `<dut>:force_all()`

   使用了 `<dut>:force_all()` 之后，接下来 `dut` 的所有赋值操作都会转化为立即生效的 `force` 类型赋值。

8. `<dut>:release_all()`

   立即释放由 `<dut>:force_all()` 建立的所有强制状态，并解除 force 模式。
...
15. `<dut>:set_force_str(str)`
    ...
    `force` 版本的 `<dut>:set_str(str)`，立即生效。
...
28. `<dut>:set_freeze()`
    ...
    立即读取并强制保持当前信号值。需要调用 `<dut>:set_release()` 来取消冻结。
