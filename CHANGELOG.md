...
- **libverilua**: Lua time accounting (`lua_time_taken` / `lua_overhead` in the exit statistics table) is now a runtime switch instead of the compile-time `acc_time` cargo feature (removed). Default is off: the table shows `--` and a dim hint below explains how to enable. Set `VL_ACC_LUA_TIME=1` to get the numbers for a single run without rebuilding. Skipping the two per-event `clock_gettime` calls when off saves ~3-5% CPU time on edge-callback-dense workloads.
...
- **dpi_exporter**: New `meta_only = true` group flag in `add_pattern`: matched signals are registered in the meta table (width/type available to `CallableHDL`) but get no DPI accessor, tick sampling, or write function, and are pinned through a generated `dpi_exporter.public.vlt` (verilator `public_flat_rd`; pass it to verilator with the rewritten RTL, other simulators ignore it). Value access is left to the runtime — mem_direct on verilator, a real VPI handle elsewhere; binding fails loudly when neither exists. Mutually exclusive with `writable` / `writable_signals` / `sensitive_signals`. `DpiExporter:lookup` also accepts `TOP.`-prefixed fullpaths now (same normalization as mem_direct).
- **mem_direct (Verilator)**: Add optional native design-state access through `rootp + offset`. Enable with `verilua.verilator_mem_direct` or build-time `VL_XMK_VERILATOR_MEM_DIRECT=0|1`; the entry table is compiled into a standalone `libmem_direct.so` loaded at runtime via `VL_MEM_DIRECT_SO` (set automatically by the xmake rule), and verilua auto-initializes it during `init.lua` — no script changes needed. The `.so` self-locates the design state through Verilator's public scope registry, so simulation binaries need no contract code. Offsets are emitted as `offsetof` constants split into per-chunk TUs compiled in parallel, keeping `.so` build time around tens of seconds even for SoC-scale tables. Optionally limit generated entries with `verilua.verilator_mem_direct_include` / `_exclude`. Default `set` remains on VPI/pending. See `docs/reference/mem_direct.mdx`.
- **vl-verilator wrappers**: `vl-verilator` / `vl-verilator-p` / `vl-verilator-dpi` support `--vl-mem-direct` (plus repeatable `--vl-mem-direct-include` / `--vl-mem-direct-exclude`, same pattern syntax as the xmake values) to generate and compile `libmem_direct.so` automatically after a successful verilator run; the flags are stripped before invoking verilator (adding `-LDFLAGS -rdynamic` so the `.so` can resolve Verilated symbols) and a `export VL_MEM_DIRECT_SO=...` hint is printed.
...
- **LuaDataBaseV2**: Database-folder creation no longer fails with `Cannot create folder: ... err: File exists` when parallel simulations create the same db path concurrently (TOCTOU between the existence probe and `mkdir`). Losing the mkdir race is now tolerated; real failures (permission, missing parent, path occupied by a file) still fail loudly.
...
- **testbench_gen**: Clock/reset auto-detection now ranks exact names before fuzzy prefix/suffix matches, rejects ambiguous same-priority candidates with an explicit `--clock-signal` / `--reset-signal` prompt, and validates explicitly selected ports. This prevents ports such as `io_sys_snapshot_reset` from shadowing a later exact `reset` port.
