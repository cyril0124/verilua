...
- **signal_db_gen**: VPI type now comes from the symbol kind resolved in C++ instead of Lua pattern-matching on `getType().toString()`: `VariableSymbol` reports `vpiReg` and `NetSymbol` reports `vpiNet`, matching `dpi_exporter`. Behavior change: `logic`/`bit` variables are now reported as `vpiReg` (previously `vpiNet`); `reg` and `wire` keep their previous values.
