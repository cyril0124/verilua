...
target("signal_operation", function()
    ...
    set_values("verilua.build_dir_name", "signal_operation")
    set_values("verilua.lua_main", path.join(curr_dir, "cases", "signal_operation.lua"))
end)
...
target("multitasking", function()
    ...
    add_files(path.join(rtl_dir, "top.sv"))
    set_values("verilua.top", "top")
    set_values("verilua.build_dir_name", "multitasking")
    set_values("verilua.lua_main", path.join(curr_dir, "cases", "multitasking.lua"))
end)
...
target("matrix_multiplier", function()
    ...
    set_values("verilua.top", "matrix_multiplier")
    set_values("verilua.build_dir_name", "matrix_multiplier")
    set_values("verilua.lua_main", path.join(curr_dir, "cases", "matrix_multiplier.lua"))
end)
...
target("matrix_multiplier_no_internal_clock", function()
    ...
    set_values("verilua.build_dir_name", "matrix_multiplier_no_internal_clock")
    set_values("verilua.lua_main", path.join(curr_dir, "cases", "matrix_multiplier_no_internal_clock.lua"))
end)
...
target("wave_vpi_gen", function()
    ...
    set_values("verilua.top", "wave_vpi_bench")
    set_values("verilua.build_dir_name", "wave_vpi_gen")
    set_values("verilua.lua_main", path.join(curr_dir, "cases", "wave_vpi_gen.lua"))

    after_run(function()
        local build_dir = path.join(curr_dir, "build", "verilator", "wave_vpi_gen")
        local fst_file = path.join(build_dir, "bench.fst")
        assert(os.isfile(fst_file), "bench.fst not found after wave_vpi_gen run")
        local waves_dir = path.join(curr_dir, "waves")
        os.mkdir(waves_dir)
        os.cp(fst_file, waves_dir)
    end)
end)
...
target("wave_vpi_bench", function()
    ...
    set_values("verilua.build_dir_name", "wave_vpi_bench")
    set_values("verilua.lua_main", path.join(curr_dir, "cases", "wave_vpi_bench.lua"))
end)
...
target("wave_vpi_gen_vcd", function()
    ...
    set_values("verilua.top", "wave_vpi_bench")
    set_values("verilua.build_dir_name", "wave_vpi_gen_vcd")
    set_values("verilua.lua_main", path.join(curr_dir, "cases", "wave_vpi_gen.lua"))

    after_run(function()
        local build_dir = path.join(curr_dir, "build", "verilator", "wave_vpi_gen_vcd")
        local vcd_file = path.join(build_dir, "bench.vcd")
        assert(os.isfile(vcd_file), "bench.vcd not found after wave_vpi_gen_vcd run")
        local waves_dir = path.join(curr_dir, "waves")
        os.mkdir(waves_dir)
        os.cp(vcd_file, waves_dir)
    end)
end)
...
target("wave_vpi_bench_vcd", function()
    ...
    set_values("verilua.top", "tb_top")
    set_values("verilua.build_dir_name", "wave_vpi_bench_vcd")
    set_values("verilua.lua_main", path.join(curr_dir, "cases", "wave_vpi_bench.lua"))
end)
...
target("wave_vpi_gen_fsdb", function()
    ...
    add_files(path.join(wave_vpi_rtl_dir, "wave_vpi_bench.sv"))
    set_values("verilua.top", "wave_vpi_bench")
    set_values("verilua.build_dir_name", "wave_vpi_gen_fsdb")
    set_values("verilua.lua_main", path.join(curr_dir, "cases", "wave_vpi_gen.lua"))
    ...
    after_run(function()
        ...
        local waves_dir = path.join(curr_dir, "waves")
        os.mkdir(waves_dir)
        assert(os.isfile(fsdb_file), "bench.vcd.fsdb not found after wave_vpi_gen_fsdb run")
        os.cp(fsdb_file, waves_dir)
    end)
end)
...
target("wave_vpi_bench_fsdb", function()
    ...
    set_values("verilua.top", "tb_top")
    set_values("verilua.build_dir_name", "wave_vpi_bench_fsdb")
    set_values("verilua.lua_main", path.join(curr_dir, "cases", "wave_vpi_bench.lua"))
end)
