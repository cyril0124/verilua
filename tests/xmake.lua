...
add_group_target("test-dpi-exporter-chdl", function(ctx)
    ctx.run_case("test_dpi_exporter_chdl", function()
        local cwd = path.join(ctx.tests_dir, "test_dpi_exporter_chdl")
...
add_group_target("test-cov-exporter-dynamic", function(ctx)
    ctx.run_case("test_cov_exporter_dynamic", function()
        local cwd = path.join(ctx.tests_dir, "test_cov_exporter_dynamic")
        ctx.run_cmd(cwd, "xmake build -P .")
        ctx.run_cmd(cwd, "xmake run -P .")
    end)
end)
