...
describe("IDPool test", function()
    ...
    it("should fail when pool is empty", function()
        ...
        expect.fail(function()
            local _ = pool:alloc()
        end, "[IDPool] pool is empty")
    end)

    it("should not corrupt pool state when alloc fails on empty pool", function()
        local pool = IDPool(2)
        local id1 = pool:alloc()
        local id2 = pool:alloc()
        expect.equal(pool:free_count(), 0)
        ...
        assert(not ok)
        -- Failed alloc must leave free/used counts unchanged.
        expect.equal(pool:free_count(), 0)
        expect.equal(pool:used_count(), 2)
        ...
        expect.equal(pool:used_count(), 1)
        local id3 = pool:alloc()
        expect.equal(id3, id1)
        pool:release(id2)
        pool:release(id3)
        expect.equal(pool:free_count(), 2)
        expect.equal(pool:used_count(), 0)
    end)
